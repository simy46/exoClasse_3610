...
void TaskActive(void* data)
{
	...
	while(1)
	{
		...
		UCOS_Print("Attente active de 1000 ticks \r\n");
		...
		delay_float = (float) delay / (float) freq_hz;

		xil_printf("Fin tache active - OSTickCtr: %d et timestamp ma conversion: ", OSTimeGet(&err));
		printf("'%.6f'", delay_float);
...
void TaskNonActive(void* data)
{
	...
	while(1)
	{
		...
		UCOS_Print("Attente non-active de 1000 ticks\r\n");
//		OSTimeDly(1000,OS_OPT_TIME_DLY, &err);
		OSTimeDly(1000,OS_OPT_TIME_PERIODIC, &err);


//		UCOS_Print("Attente non-active de 5 secondes\r\n");
